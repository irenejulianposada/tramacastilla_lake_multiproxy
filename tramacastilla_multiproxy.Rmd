---
title: "tramacastilla_multiproxy"
author: "irenejulianposada"
date: "2025-07-11"
...
load("tram20_multiproxy.RData")
```

# 1. Load ages and depths

```{r}
head(tram20_depth_ages)
...
## 2.1. Data upload
```{r}
head(charcoal.tram) #charcoal data from Tramacastilla
head(samples_names) #names of charcoal samples
head(Tramacastilla_age)
```
I need to add samples that are not here because i should have 202 and i have 194, as they did not have charcoal particles
```{r}
...
length(unique(charcoal.tram.202$SampleId)) #make sure there are all 202 samples
```
...
length(unique(charcoal.tram.all$SampleId)) #make sure there are all 202 samples
```
I want to create 2 dataframes: one for global and another for seedle
```{r}
...
We are going to filter particles so we remove all that are below 110 um, that we assume is the noisy part of the signal --> I want to remove those charcoal with an area <110 um
```{r}
...
```
Sum terrestrial PAR in each sample to see relationship between PAR (terrestrial vegetation) and CHAR
```{r}
